# Partial Retreat: Reverting Without Losing Progress

Use when a completed task is found to be faulty but other completed tasks are sound.

1. Gandalf identifies the faulty task and its downstream dependents.
2. Gandalf marks the faulty task as `in_progress` and all dependents as `pending`.
